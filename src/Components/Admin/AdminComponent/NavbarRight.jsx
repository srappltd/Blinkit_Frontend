...
import { asyncAdminLogout } from '../../../Store/Actions/AdminAction'

const NavbarRight = ({menuSlide,menuCloseBtn,menuLeft}) => {
    const dispatch = useDispatch(null)
    const navigate = useNavigate()
    const logoutAdminHandler = () =>{
        dispatch(asyncAdminLogout(navigate))
  ...
  return (
    <nav className='w-full h-[8vh] border-b  flex items-center justify-between lg:px-10 px-5 text-xl bg-white '>
                <div className='flex items-center lg:gap-3 gap-2'>
                    ...
                    <i onClick={menuCloseBtn} className="ri-skip-right-line lg:text-2xl cursor-pointer lg:hidden block"></i>
                </div>
                ...
                <div className={` text-sm font-semibold rounded bg-white text-zinc-500 shadow-[0px_0px_5px_rgba(210,210,210,5)] ${menuLeft ? 'block' : 'hidden'} fixed top-[9vh] z-30 lg:right-10 right-5`}>
                    ...
                    <div onClick={logoutAdminHandler} className='border-b cursor-pointer h-11 items-center flex px-3'>Sign out</div>
                </div>
            </nav>
  )
}
